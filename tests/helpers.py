...
from app.models import Usage, UsageTypes, User
...
def create_usage(user_id, usage_type_id, usage_at, amount):
    usage = Usage.objects.create(user_id=user_id, usage_type_id=usage_type_id, usage_at=usage_at, amount=amount)
    return usage


def create_usage_types(name, unit, factor):
    usage_type = UsageTypes.objects.create(name=name, unit=unit, factor=factor)
    return usage_type
